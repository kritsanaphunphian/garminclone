...
foreach ( $items as $item_id => $item ) :
	...
	$_partnumber_text_field_variation = get_post_meta( $product_variation_id, '_partnumber_text_field_variation', true );
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
			<td class="td" style="text-align:<?php echo $text_align; ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; word-wrap:break-word;">
				...
				if( "" != $product_variation_id && "" != $product_id ) {
					echo apply_filters( 'woocommerce_order_item_name', '<br>Part Number : ' . $_partnumber_text_field_variation , $item, false );
				} else {
					echo apply_filters( 'woocommerce_order_item_name', '<br>Part Number : ' . $_partnumber_product_field_simple , $item, false );
				}
