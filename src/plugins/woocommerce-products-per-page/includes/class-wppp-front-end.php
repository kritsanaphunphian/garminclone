...
class WPPP_Front_End {
	...
	public function products_per_page_dropdown() {
		...
		?><form method="<?php echo esc_attr( $method ); ?>" action="<?php echo esc_url( $action ); ?>" class="woocommerce-ordering"><?php

			 do_action( 'wppp_before_dropdown' );

			?><select name="ppp" onchange="this.form.submit()" class="select wppp-select from-align"><?php
