...
if ( ! class_exists( 'YITH_WCMAP_Frontend' ) ) {
	...
	class YITH_WCMAP_Frontend {
		...
		public function check_myaccount() {
			...
			// add Elementor Page Builder check
			if( $this->_is_myaccount && defined('ELEMENTOR_VERSION') && get_post_meta( $post->ID, '_elementor_edit_mode', true ) === 'builder' ) {
				// remove default actions
				remove_action( 'template_redirect', array( $this, 'add_account_menu' ), 151 );
				remove_action( 'template_redirect', array( $this, 'add_custom_endpoint_content' ), 152 );
				// add Elementor Page Builder compatibility
				add_filter( 'elementor/frontend/builder_content_data', array( $this, 'add_widget_custom_content' ), 10, 1 );
			}
		...
		/**
		 * Add plugin content to Elementor widget content before render
		 *
		 * @since 2.2.8
		 * @author Francesco Licandro
		 * @param array $data
		 * @return array
		 */
		public function add_widget_custom_content( $data ){
			return $this->_is_myaccount ? $this->replace_recursive( $data ) : $data;
		}

		/**
		 * Replace recursive array Elementor content
		 *
		 * @since 2.2.8
		 * @author Francesco Licandro
		 * @param array $array
		 * @return array
		 */
		protected function replace_recursive( &$array ){
			if( isset( $array['settings']['editor'] ) ) {
				//extract from widget content the my-account shortcode
				$data = $array['settings']['editor'];
				preg_match( '/\[woocommerce_my_account[^\]]*\]/', $data, $shortcode_myaccount );
				if ( isset( $shortcode_myaccount[0] ) ) {
					$default_myaccount_content = $content = $shortcode_myaccount[0];
					...
					// set new content
					$new_myaccount_content = '<div id="my-account-content" class="woocommerce woocommerce-MyAccount-content">' . $content . '</div>';
					$new_myaccount_content = ( $position == 'right' && $tab == '' ) ? $new_myaccount_content . $menu : $menu . $new_myaccount_content;
					$array['settings']['editor'] = str_replace( $default_myaccount_content, $new_myaccount_content, $data );
					return $array;
				}
			}
			...
			if( is_array( $array ) ){
				foreach( $array as &$child ){
					$child = self::replace_recursive( $child );
				}
			}
